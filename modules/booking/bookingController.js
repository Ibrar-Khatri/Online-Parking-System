  ...
  db.collection("locations").doc(locationId).delete()
    .then(locationDeleted => {
      db.collection("bookings")
        ...
        .get()
        .then(async (bookings) => {
          let removedUpcomingBookings = []
          await bookings.forEach((bking) => {
            db.collection("bookings").doc(bking.id).delete()
            removedUpcomingBookings.push(bking.id)
          });
          res.send({
            status: true, message: 'Parking area removed successfully', removedBookingsId: removedUpcomingBookings
          })
        })
        .catch((error) => {
          res.send({
